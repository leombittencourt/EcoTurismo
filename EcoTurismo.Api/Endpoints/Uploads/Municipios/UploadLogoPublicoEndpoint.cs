...
public class UploadLogoPublicoEndpoint : Endpoint<UploadLogoPublicoRequest>
{
    ...
    public UploadLogoPublicoEndpoint(EcoTurismoDbContext db, IImageService imageService)
    {
        _db = db;
    ...
    public override async Task HandleAsync(UploadLogoPublicoRequest req, CancellationToken ct)
    {
        ...
        try
        {
            ...
            if (municipio.LogoAreaPublicaId.HasValue)
            {
                await _imageService.RemoverImagemAsync(municipio.LogoAreaPublicaId.Value);
            }

            // Atualizar município com a nova imagem
            municipio.LogoAreaPublicaId = result.Data!.Id;
